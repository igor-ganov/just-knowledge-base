...
- [x] T21 — Passkey protector: WebAuthn PRF -> HKDF -> KEK wrapping the shared DEK; create-time opt-in, passkey unlock button, late enrollment from settings with password; password always remains the fallback (AC-1.0..1.0c) -> unit: protectors.test.ts; E2E: passkey.spec.ts with CDP virtual authenticator (hasPrf).
